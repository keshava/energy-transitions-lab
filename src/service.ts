...
import { XrathusApi, ApiToken, DatasetItem, DatasetFile } from "./xrathus";

interface ApiTokenObject extends ApiToken, JSONObject {}

export class XrathusService {
  static NAMESPACE = "xrathus";
  static ROOT_PATH = "xrathus";
  static WORK_PATH = "work";
  static PLUGIN_ID = "@xrathus/jupyterlab:xrathus";
  static XRATHUS_API_URL = XrathusApi.XRATHUS_API_URL;
  static XRATHUS_BASE_URL = "https://www.xrathus.com";
  ...
  constructor(
    ...
    this.getApiToken().then(apiToken => {
      if (
        ...
        const kapi2 = new XrathusApi(apiToken);
        if (kapi2.hello()) {
          this._ready = true;
          this._kapi = kapi2;
  ...
  public async getApiToken(): Promise<ApiToken> {
    const apiToken = (await this._stateDB.fetch(
      `${XrathusService.PLUGIN_ID}:apiToken`
    )) as ApiTokenObject;
    return apiToken;
  }

  public getOwnerUrl(data: DatasetItem): string {
    return data && "https://www.xrathus.com/" + data.ownerRef;
  }
  ...
  public listDatasets(page?: number, search?: string) {
    if (!this.isReady()) {
      throw new Error("XrathusApi not initialized.");
    }
  ...
  public async addDataset(dataset: DatasetItem): Promise<DatasetFile[]> {
    ...
    await this._drive
      .get(XrathusService.ROOT_PATH, { content: false })
      .catch(async reason => {
        const e = reason as Error;
        if (e && e.message == "Invalid response: 404 Not Found") {
          console.log("Creating root folder for", XrathusService.ROOT_PATH);
          await this._drive.save(XrathusService.ROOT_PATH, {
            path: "",
            type: "directory",
          });
        }
  ...
  public async createNotebook(dataset: DatasetItem): Promise<void> {
    await this._drive
      .get(XrathusService.WORK_PATH, { content: false })
      .catch(async reason => {
        const e = reason as Error;
        if (e && e.message == "Invalid response: 404 Not Found") {
          console.log("Creating working folder for", XrathusService.WORK_PATH);
          await this._drive.save(XrathusService.WORK_PATH, {
            path: "",
            type: "directory",
          });
        }
    ...
    const notebook = await this._manager.newUntitled({
      path: XrathusService.WORK_PATH,
      type: "notebook",
    });

    const xrathusPath: string =
      "/" +
      PathExt.join(PageConfig.getOption("serverRoot"), XrathusService.ROOT_PATH);
  ...
  private async ensureFolder(
    ...
    const fullPath = PathExt.join(XrathusService.ROOT_PATH, basePath, directory);
